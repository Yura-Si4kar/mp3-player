    ...
    return (
        <Modal show={show} onHide={hide}>
            <Modal.Header className='d-flex justify-beetwen'>
                ...
                <MyButton variant='outline-light' onClick={hide}>&#10060;</MyButton>
            </Modal.Header>   
            <Modal.Body>
                <ListGroup>
                    {player.list.map((el) => <AudioItem key={el.id} show={show} audio={el} index={player.list.indexOf(el)} addAudioToAlbum={addAudioToAlbum} />)}  
                </ListGroup>
            </Modal.Body>
        </Modal>
    )
